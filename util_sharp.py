...
class CIFAR10_w_indices(datasets.CIFAR10):
    def __init__(self, root: str, train: bool = True, transform = None, target_transform = None,download=False,train_ratio=1.0,front=True):
        super().__init__(root, train, transform, target_transform,download)
        self.train_ratio = train_ratio
        clean_data_num = int(len(self.data)*train_ratio)
        if front:
            self.data = self.data[:clean_data_num]
            self.targets = self.targets[:clean_data_num]
        else:
            self.data = self.data[clean_data_num:]
            self.targets = self.targets[clean_data_num:]
    def __len__(self):
...
class Dataset_load(torch.utils.data.Dataset):
    def __init__(
        ...
        self.samples.sort(key=lambda x: int(x[:-4]))
        self.root = root
...
def get_dataset(args, transform_train,train_ratio=1.0,front=True):
...
def get_loader(args, train_dataset, test_dataset):
    if args.dataset == "imagenet100":
        ...
        train_loader = torch.utils.data.DataLoader(
            train_dataset, batch_size=args.bs, shuffle=True, num_workers=4, drop_last=True # Train needs shuffle
        )
        test_loader = torch.utils.data.DataLoader(
